...
- `init --from-compose` now warns when a service's `env_file` names a file that
  is not there. The reference is imported rather than the contents, on purpose —
  inlining the variables would copy whatever is in that file, credentials
  included, into a generated config about to be committed beside the Compose
  file it came from — but that means a missing file is not discovered until the
  run refuses the config, several steps and one image pull later. Compose
  projects commonly ship a `.env` that is deliberately absent from the
  repository, so the file is missing exactly when the config is generated by
  somebody who has not seen the project before.
