...
small preflightkit profile the selected service needs.
"""
...
from preflightkit.config.loader import ConfigError
...
def _service(
    ...
    if not image:
        if build is not None:
            warnings.append(
                f"service {name} has build but no image; preflightkit does not "
