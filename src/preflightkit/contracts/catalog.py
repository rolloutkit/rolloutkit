...
CATALOG: dict[str, ContractDoc] = {
    ...
    "SP005": ContractDoc(
        ...
        (
            ...
            "When readiness is the zero-config fallback target it has to clear two "
            "independent clauses, and both must hold. Its p50 must be at least "
            "10x the probe-path jitter, so the before/after-SIGTERM boundary is "
            "distinguishable from noise, and it must be at least 3ms in absolute "
            "terms whatever the ratio says. An explicit --inflight-path bypasses "
            "both.",
            "Neither clause is the junior one, and which of the two actually "
            "refuses a given service depends on the machine the run happened on. "
            "Measured over eight runs of one fixture on each of two hosts. On an "
            "idle macOS laptop the probe path is quiet, so the ratio comes close "
            "to clearing — it reached 4.09 against the cutoff of 10 — and what "
            "refuses is the absolute floor: readiness p50 never rose above "
            "0.79ms, 3.8x under the 3ms it needs. On a two-core Linux CI runner "
            "the jitter is three times larger, so the ratio refuses with room to "
            "spare at 2.41 and below, 4.2x under its cutoff, while the same "
            "fixture's readiness p50 climbs to 1.19ms, within 2.5x of the floor. "
            "Remove either clause and one of those two hosts loses the clause "
            "that was holding it. The ratio is evaluated first, so when both "
            "refuse the reported cause names the ratio; that is an ordering, not "
            "a ranking.",
