...
from rolloutkit.config.duration import format_measured_ms
from rolloutkit.config.models import Platform
from rolloutkit.contracts.base import ContractResult, Status
from rolloutkit.engine.context import SIGKILL_EXIT, SIGTERM_EXIT, RunReport
...
def _evidence(report: RunReport) -> dict[str, Any]:
    ...
    return {
        **_static_evidence(report),
        ...
        # `sigkill_sent` is what rolloutkit did; `sigkill_effective` is what
...
def _handler_note(report: RunReport) -> list[str]:
    if report.runtime_handler_installed is None:
        return [
            ...
            "`busybox:latest` image is present locally, and rolloutkit does not "
...
def _static_notes(report: RunReport, static: dict[str, Any]) -> list[str]:
    ...
    if report.sigkill_sent and not report.sigkill_effective:
        notes.append(
            "rolloutkit sent SIGKILL when the budget ran out, but the process "
    ...
    notes.append(
        "rolloutkit never enables Docker's --init: tini as PID 1 would change "
