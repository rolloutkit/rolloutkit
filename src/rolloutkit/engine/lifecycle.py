...
from rolloutkit.traffic.baseline import (
    BASELINE_SAMPLES,
    measure_baseline,
...
from rolloutkit.contracts.inflight import fallback_resolution_cause
...
async def run_experiment(
    ...
    network = await runtime.create_network(f"rk-{run_token}")
        ...
        for service_name, service in config.services.items():
            dependency = await runtime.start(
                ContainerSpec(
                    ...
                    name=f"rk-{run_token}-{service_name}",
                    network_name=network.name,
