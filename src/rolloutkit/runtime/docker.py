...
from rolloutkit.runtime.socket import DockerUnavailable, Endpoint, discover_endpoint

MIN_API_VERSION = (1, 41)
LABEL_OWNER = "io.rolloutkit.owned"

#: Image used for the two auxiliary containers described below. Never pulled:
#: rolloutkit does not fetch images on the user's behalf, and a probe that
...
def launch():
    time.sleep(0.05)
    os.execve("/usr/local/bin/python", ["python", "-B", "-u", "/opt/rk_probe/sidecar_entry.py"], {"PYTHONPATH": "/opt/rk_probe/vendor"})
...
class ProbePackagingError(Exception):
    ...
    This is the other kind. A module rolloutkit ships against is missing, so no
...
class DockerRuntime:
    ...
    async def ping_latency_ns(self, samples: int = 5) -> list[int]:
        ...
        """
        from rolloutkit.engine.events import now_ns
    ...
    async def start_traffic_probe(
        ...
        dependencies into a bounded tmpfs. No registry-hosted rolloutkit image,
...
#: Pure-Python dependencies copied into the probe beside rolloutkit's own
...
#: being an anyio requirement, so a clean `pip install rolloutkit` has none.
...
def _missing_dependency(package: str) -> str:
    return (
        ...
        f"rolloutkit does not have it. That is the installation, not this "
        ...
        f"host-side measurement as if you had chosen it. Reinstall rolloutkit "
        f"(`pip install --force-reinstall rolloutkit`)."
    )
...
def _add_vendored(archive: tarfile.TarFile, package: str, *, required: bool) -> None:
    ...
    archive.add(
        ...
        arcname=f"rk_probe/vendor/{package}",
        filter=_drop_cache,
    )
...
def _traffic_probe_archive() -> bytes:
    ...
    with tarfile.open(fileobj=output, mode="w") as archive:
        for source in files:
            ...
            destination = (
                Path("rk_probe/sidecar_entry.py")
                if relative == Path("runtime/sidecar_entry.py")
                else Path("rk_probe/vendor/rolloutkit") / relative
            )
        ...
        archive.add(
            ...
            arcname="rk_probe/vendor/typing_extensions.py",
            recursive=False,
        )
...
def _traffic_probe_body(image: str, network_name: str, name: str) -> dict[str, Any]:
    ...
    return {
        ...
        "HostConfig": {
            ...
            "ReadonlyRootfs": True,
            "Tmpfs": {
                "/opt/rk_probe": "rw,nosuid,nodev,noexec,size=16m,mode=0755"
            },
        ...
        "NetworkingConfig": {
            "EndpointsConfig": {
                network_name: {"Aliases": ["rolloutkit-probe"]}
            }
        },
    }
