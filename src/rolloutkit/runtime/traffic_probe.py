...
from rolloutkit.engine.events import RequestPhase, now_ns
from rolloutkit.probes.http import ProbeResult
from rolloutkit.runtime.base import Container, ContainerSpec, TeardownCalibration
...
class TrafficProbe:
    ...
    async def measure_teardown_floor(
        ...
        for index in range(samples):
            alias = f"rk-floor-{index}-{time.time_ns()}"
